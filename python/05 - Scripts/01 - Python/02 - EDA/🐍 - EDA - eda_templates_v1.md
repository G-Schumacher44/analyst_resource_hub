...
if __name__ == "__main__":
    ...
    data_path = "../data/synthetic_penguins_v3.4.csv"

	# target a column
	target_col = "your_target_variable_here"
    ...
    skewness_summary = calculate_skewness_scipy(df, numeric_cols)
    print("\n🔎 Skewness Summary:\n", pd.Series(skewness_summary))

    # Plot Correlation Matrix
    plot_correlation_matrix(df, numeric_cols)

    # Plot Boxplots with Outliers
    plot_boxplots_with_outliers(df, numeric_cols)

    # Check for High Cardinality in Categorical Columns
    check_high_cardinality(df, categorical_cols)

    # Plot Scatter Matrix
    plot_scatter_matrix(df, numeric_cols)

    # Detect Outliers (choose method: 'iqr' or 'zscore')
    detect_outliers(df, numeric_cols, method='iqr')

    # Plot Q-Q plots for normality checks
    plot_qq(df, numeric_cols)

    # Plot Boxplots for Categorical vs Numeric
    boxplot_cat_vs_numeric(df, categorical_cols, numeric_cols)

	# Plot Scatterplots with Regression Lines (Pre-Modeling Visual Check)
	for num_col in numeric_cols:
    scatter_with_regression(df, num_col, target_col)

    # OPTIONAL: Plot feature vs target boxplots if target available
    target_col = "species"  # Example target (adjust based on your data)
...
# **📜** Scatterplot with Regression Line(pre-model)
```python
...
## 🔎 Outlier Detection and Kurtosis
